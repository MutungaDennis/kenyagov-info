...
export default function CabinetPage() {
  ...
  useEffect(() => {
    ...
    fetch("/data/ministers.json")
      ...
      .then((data: Record<string, CabinetOfficial>) => {
  ...
  if (!registry) {
    return (
      <>
        ...
        />
        <p className="govuk-body">Loading cabinet…</p>
      </>
    );
  }
        ...
        <div className="govuk-grid-column-two-thirds">
          ...
          <ul className="govuk-list govuk-!-padding-left-0">
            {executiveLeadershipIds.map((id) => {
              const official = registry[id];
              if (!official) return null;

              const execAssignment =
                official.assignments.find((a) => a.isExecutiveOffice) ||
