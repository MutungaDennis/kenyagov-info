-- Publication is independent of lifecycle: keep published historical organisations accessible.
BEGIN;
SET LOCAL lock_timeout='5s';
ALTER POLICY public_read ON public.institutions USING (is_active IS TRUE);
ALTER POLICY authenticated_read ON public.institutions USING (is_active IS TRUE OR (SELECT private.cg_is_admin()));
DO $children$
DECLARE table_name text;
BEGIN
  FOREACH table_name IN ARRAY ARRAY['institution_name_history','institution_lifecycle_segments','institution_leaders','institution_locations'] LOOP
    EXECUTE format('ALTER POLICY public_read ON public.%I USING (EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=%I.institution_id AND i.is_active IS TRUE))',table_name,table_name);
    EXECUTE format('ALTER POLICY authenticated_read ON public.%I USING ((SELECT private.cg_is_admin()) OR EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=%I.institution_id AND i.is_active IS TRUE))',table_name,table_name);
  END LOOP;
END $children$;
ALTER POLICY public_read ON public.institution_relationships USING (
  EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=from_institution_id AND i.is_active IS TRUE)
  AND EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=to_institution_id AND i.is_active IS TRUE));
ALTER POLICY authenticated_read ON public.institution_relationships USING ((SELECT private.cg_is_admin()) OR (
  EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=from_institution_id AND i.is_active IS TRUE)
  AND EXISTS(SELECT 1 FROM public.institutions i WHERE i.id=to_institution_id AND i.is_active IS TRUE)));
CREATE OR REPLACE VIEW public.global_search_view WITH (security_invoker=true) AS
SELECT i.id::text,id_slug.slug,i.name,CASE WHEN lower(coalesce(i.status::text,'')) IN ('inactive','former','renamed','dissolved','abolished','merged','succeeded','restructured','split','absorbed','unconstitutional','judiciously annulled') THEN concat('Historical organisation (',i.status,'). ',i.description) ELSE i.description END AS snippet,'Institution'::text AS entity_type,
  '/government/institutions'::text AS base_route,i.search_vector,CASE WHEN lower(coalesce(i.status::text,'')) IN ('inactive','former','renamed','dissolved','abolished','merged','succeeded','restructured','split','absorbed','unconstitutional','judiciously annulled') THEN 2 ELSE coalesce(i.search_boost,10) END AS rank_boost,
